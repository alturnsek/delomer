...
- [x] "Prikaz članov": iskanje po imenu/priimku, filter po statusu (aktiven/neaktiven) in vlogi, sort A-Ž/Ž-A po imenu ali priimku (`adminMembersCache`/`renderOrgMembersList()`); neaktivni člani so vedno pomaknjeni na dno seznama ne glede na izbrano razvrščanje.
...
- [x] "Statistika društva" (ADMIN/SUPERINTENDENT) — `GET /api/admin/stats?from&to&status&category_ids&team_ids`: graf ur skozi čas (`renderHoursChart`, deljena z "Moj profil"), skupno število ur, in razčlenitev po članih (padajoče). Filtri: datumski razpon + bližnjice (enako kot Potrjevanje dela), status (privzeto Potrjeno), večizborne kljukice za kategorije in ekipe (uporabnik je predlagal "radio button" za več-izbiro, kar tehnično ni mogoče - implementirano kot checkbox seznam z enako UX logiko).
- [ ] "Statistika društev" (SUPER_ADMIN, pregled čez vsa društva) je še vedno prazna placeholder sekcija — enak pristop kot zgoraj, samo brez `organization_id` omejitve in z razčlenitvijo po društvih namesto po članih.
