...
- [x] **Landing stran na `delomer.top`/`www.delomer.top`**: marketinška stran (opis aplikacije, funkcije, kiosk, kontakt) v [src/landing/](src/landing/index.html) (samostojen HTML/CSS, brez odvisnosti od SPA). Namesto ločenega deploy-a je **isti Express kontejner** dobil hostname-based routing middleware v [src/app.js](src/app.js): zahteve s `Host: delomer.top`/`www.delomer.top` (konfigurabilno preko `LANDING_HOSTS` env spremenljivke) postrežejo `src/landing/`, vse ostalo (vključno z `app.delomer.top`) gre naprej na SPA/API kot doslej. Nepoznana pot na landing gostitelju vedno pade nazaj na `landing/index.html` (enostranska stran, brez routinga), nikoli ne pade skozi na API. Vizualno preverjeno s Playwright posnetki (namizje + mobilni).
  - **Potrebna infrastrukturna sprememba na TrueNAS** (ne samo `git pull`): v Nginx Proxy Manager dodaj nov Proxy Host za `delomer.top` + `www.delomer.top`, ki kaže na isti kontejner/port kot obstoječi `app.delomer.top` vnos; v Cloudflare dodaj/preveri DNS zapisa za koren domene in `www`; preveri, da Cloudflare Origin certifikat pokriva tudi `delomer.top` (ne samo `*.delomer.top`), sicer ga je treba razširiti.
...
- [x] **Responsive design za tablice/mobilne naprave**: manjkal je `<meta name="viewport">` na vseh treh HTML straneh (`index.html`/`join.html`/`set-password.html`) - brez njega mobilni brskalniki celotno stran renderirajo na navidezni namizni širini (~980px) in jo pomanjšajo, zato so bile vse spodnje spremembe pred tem brez učinka na pravi napravi. Dodano: en sam preboj pri 768px (`@media (max-width: 768px)`) - `.row`/`.actions`/`.edit-actions`/`.confirm-actions` zdaj vedno `flex-wrap` (prej edini flex brez wrapa v aplikaciji, silil je vsebino v vodoravni prelivi), manjši header/logotip/sidebar (postane 80vw/max 280px "drawer" namesto 220px), `input`/`select` na 16px (iOS Safari sicer samodejno približa stran ob fokusu pod 16px). Meni (sidebar) se na mobilnem/tablici privzeto začne **zaprt** (na namizju ostane shranjena `localStorage` preferenca), se samodejno zapre po izbiri strani iz menija, in se zapre s klikom kamorkoli izven njega (`isMobileViewport()`/`MOBILE_BREAKPOINT` v script.js). Vizualno preverjeno z avtomatiziranim Playwright posnetkom zaslona na 375px in 768px širini (lokalni Express strežnik brez baze, DOM stanje prijavljenega uporabnika simulirano ročno) - med tem odkrit in popravljen pravi bug: `#headerUser` pozdravno besedilo se je na ozkem zaslonu prekrivalo s centriranim logotipom (oba `position:absolute`), zdaj se na mobilnem/tablici skrije (ostane samo gumb "Odjava").
